...
import { Sun, Moon, LogOut, Search, RotateCcw, Loader2, ChevronDown } from 'lucide-react';
...
export default function DashboardPage() {
  const { logout, firebaseUser, user } = useAuth();
          ...
          <div className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-8">
            ...
            <div className="flex items-center gap-3">
              <a
                ...
                className="rounded-xl border border-border bg-card px-4 py-2.5 mr-2 text-sm font-semibold shadow-sm transition-all hover:bg-primary/10 hover:text-primary hover:border-primary/30 active:scale-95"
              >
                Manual de procedimiento
              </a>
              <div className="hidden flex-col items-end mr-2 md:flex">
                <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground/60">Usuario</span>
                <span className="text-sm font-medium">{firebaseUser?.email}</span>
              </div>
              <ModeToggle />
              <button
                onClick={logout}
                className="flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-2.5 text-sm font-semibold shadow-sm transition-all hover:bg-destructive/10 hover:text-destructive hover:border-destructive/30 active:scale-95"
              >
          ...
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                className="rounded-xl border border-border bg-card px-4 py-2 text-sm font-medium shadow-sm ring-offset-background transition-all focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
              >
...
function ModeToggle() {
  return (
    <button
      ...
      className="rounded-xl border border-border bg-card p-2.5 shadow-sm transition-all hover:bg-muted hover:border-primary/20 active:scale-95"
      title="Cambiar tema"
    >
